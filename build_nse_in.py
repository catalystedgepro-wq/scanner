#!/usr/bin/env python3
"""build_nse_in.py — India NSE catalyst news via yfinance per .NS ticker.

Output: docs/nse_in.csv + docs/data/nse_panels.json
"""
...
def _find_root() -> Path:
    for cand in (Path("/opt/catalyst"),
        ...
        if (cand / "build_nse_in.py").exists():
            return cand
    return Path(__file__).resolve().parent


ROOT = _find_root()
OUT_CSV = ROOT / "docs/nse_in.csv"
OUT_JSON = ROOT / "docs/data/nse_panels.json"
...
TICKERS = [
    ...
    ("ITC.NS", "ITC"), ("SBIN.NS", "State Bank India"),
    ("BHARTIARTL.NS", "Bharti Airtel"), ("KOTAKBANK.NS", "Kotak Mahindra"),
    ("LT.NS", "Larsen & Toubro"), ("ASIANPAINT.NS", "Asian Paints"),
    ...
    ("NESTLEIND.NS", "Nestle India"), ("TATAMOTORS.NS", "Tata Motors"),
    ("SUNPHARMA.NS", "Sun Pharma"), ("POWERGRID.NS", "Power Grid"),
    ("NTPC.NS", "NTPC"),
]

KIND_PATTERNS = [
    ("results",       r"results|earnings|profit|quarterly|annual"),
    ("dividend",      r"dividend|distribution|interim"),
    ("buyback",       r"buyback|repurchase"),
    ("merger",        r"merger|acquisition|takeover|stake"),
    ("regulatory",    r"sebi|regulator|investigation|fine"),
    ("guidance",      r"guidance|outlook|forecast"),
    ("rating",        r"upgrade|downgrade|rating"),
]
...
def main():
    ...
    OUT_JSON.write_text(json.dumps({
        ...
        "source": "Yahoo Finance news per NSE ticker",
        "exchange": "NSE", "country_iso": "IND",
        "count": len(rows), "by_kind": dict(by_kind),
        "recent": rows[:50],
    }, indent=2))
    print(f"nse_in: {len(rows)} headlines | top kinds: {sorted(by_kind.items(), key=lambda x: -x[1])[:3]}")
    return 0
