#!/usr/bin/env python3
"""build_sec_restruct.py — SEC restructuring / workforce reduction tape.

Tracks corporate pain-point actions:
- restructuring_charge — one-time P&L hit, often precedes CEO exit
  or strategic review.
- workforce_reduction / layoffs / reduction_in_force — employment
  compression signal; ADP NFP feedthrough.
- plant_closure / facility_closure — capex unwind, asset-impairment
  follow-on risk.

Economic readthrough:
- Concentrated restructuring tape = sector rolling-over signal (tech
  2022-23, retail 2024, biotech every year).
- Layoff acceleration by macro-sector = leading jobs-report indicator
  for rate-cut bid.
- Reinvestment winners: consulting (ACN/KPMG/PWC service revenue up on
  transformation work), severance insurance (liability-ballooning risk
  for thinly capitalized issuers).

Source: efts.sec.gov/LATEST/search-index
Output: sec_restruct.csv
...
def _fetch(kind: str, query: str) -> list[dict]:
    ...
    try:
        ...
        print(f"sec_restruct: {kind} fetch failed: {e}")
        return out
...
def main() -> None:
    ...
    if not rows:
        if OUT_CSV.exists() and OUT_CSV.stat().st_size > 200:
            print(f"sec_restruct: no fetch, keeping {OUT_CSV.name}")
        return
    ...
    print(f"sec_restruct: {len(rows)} rows | {cb} | "
